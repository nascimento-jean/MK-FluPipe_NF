process RUN_IRMA_LONG {
    ...
    output:
    tuple val(meta), path("irma_runs_long/${meta.id}"), emit: irma_dirs
    ...
    """
    mkdir -p irma_runs_long assembly_final reports

    IRMA "${irmaModule}" \
        "${reads[0]}" \
        "irma_runs_long/${meta.id}" \
        > "reports/${meta.id}.irma.log" 2>&1

    if ls "irma_runs_long/${meta.id}/amended_consensus/"*.fa >/dev/null 2>&1; then
