process RUN_IRMA_LONG {
    ...
    stub:
    """
    mkdir -p "irma_runs_long/${meta.id}/amended_consensus" assembly_final reports pipeline_status
    cat > "irma_runs_long/${meta.id}/amended_consensus/${meta.id}.fa" <<'FASTA'
>${meta.id}_1
ACGTACGTACGT
>${meta.id}_2
ACGTACGTACGA
...
ACGTACGTACCG
FASTA
    cp "irma_runs_long/${meta.id}/amended_consensus/${meta.id}.fa" "assembly_final/${meta.id}.fasta"
    echo "stub IRMA long log for ${meta.id}" > "reports/${meta.id}.irma.log"
    {
        printf 'sample_id\\tlayout\\tseq_type\\tread1\\tread2\\tirma_module\\tirma_dir\\tconsensus_fasta\\tlog_file\\n'
        printf '%s\\t%s\\t%s\\t%s\\t%s\\t%s\\t%s\\t%s\\t%s\\n' "${meta.id}" "${meta.layout}" "${meta.seq_type}" "${reads[0]}" "-" "${params.irma_module}" "irma_runs_long/${meta.id}" "assembly_final/${meta.id}.fasta" "reports/${meta.id}.irma.log"
    } > "${meta.id}_irma_manifest.tsv"
    {
        printf 'sample_id\\tlayout\\tseq_type\\tstatus\\treason\\tread1\\tread2\\tirma_module\\tirma_dir\\tconsensus_fasta\\tlog_file\\n'
        printf '%s\\t%s\\t%s\\t%s\\t%s\\t%s\\t%s\\t%s\\t%s\\t%s\\t%s\\n' "${meta.id}" "${meta.layout}" "${meta.seq_type}" "success" "-" "${reads[0]}" "-" "${params.irma_module}" "irma_runs_long/${meta.id}" "assembly_final/${meta.id}.fasta" "reports/${meta.id}.irma.log"
    } > "pipeline_status/${meta.id}.irma.tsv"
    """
}
