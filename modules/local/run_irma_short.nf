process RUN_IRMA_SHORT {
    ...
    publishDir "${params.output_dir}", pattern: 'irma_failures_short/*', mode: 'copy', overwrite: true
    ...
    output:
    tuple val(meta), path("irma_runs_short/${meta.id}"), emit: irma_dirs, optional: true
    ...
    if( meta.layout == 'paired' ) {
        """
        mkdir -p tmp_irma irma_runs_short irma_failures_short assembly_final reports pipeline_status

        tmp_dir="tmp_irma/${meta.id}"
        success_dir="irma_runs_short/${meta.id}"
        failure_dir="irma_failures_short/${meta.id}"
        status="success"
        reason="-"
